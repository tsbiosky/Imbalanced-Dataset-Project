...
class AlignedDataset(BaseDataset):
    def initialize(self, opt):
        ...
        self.dir_AB = os.path.join(opt.dataroot, 'images', opt.filename)
    ...
    def __getitem__(self, index):
        ...
        #print(bbox_path)
        rr=random.randint(0,2999)
        bg_path='/media/roman/storage/psgan/bg_image/'+str(rr)+'.png'
        ...
        big_img_path = bbox['image_path']
        big_img=self.load_image(big_img_path)
        bbox_list = bbox['bbox_list']
        ...
        if bbox_y <= h_offset or bbox_x <= w_offset:
            ...
            bg = bg.resize((self.opt.fineSize , self.opt.fineSize), Image.BICUBIC)
        ...
        return {'A': A, 'B': B, 'bbox': bbox,
                'A_paths': AB_path, 'B_paths': AB_path,'bg':bg,'big_img':big_img}
