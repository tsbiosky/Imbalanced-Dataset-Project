...
def define_person_D(input_nc, ndf, opt, use_sigmoid=False, gpu_ids=[]):
    ...
    netD.apply(weights_init)
    return netD
def define_person_D_AC(input_nc, ndf, opt, use_sigmoid=False, gpu_ids=[]):
    netD = None
    use_gpu = len(gpu_ids) > 0
    # print('haha')
    # print(use_sigmoid)
    ...
    if (opt.use_spp == False):
        ...
        netD = SPP_NET_AC( opt, input_nc, ndf, use_sigmoid, gpu_ids=gpu_ids)
...
class SPP_NET(nn.Module):
    ...
    def spatial_pyramid_pool(self,previous_conv, num_sample, previous_conv_size, out_pool_size):
        ...
        for i in range(len(out_pool_size)):
            ...
            if h_pad > int(h_wid / 2):
                h_pad = int(h_wid / 2)
            if w_pad > int(w_wid / 2):
                w_pad = int(w_wid / 2)
...
class SPP_NET_AC(nn.Module):
    def __init__(self, opt, input_nc, ndf=64, use_sigmoid=False, gpu_ids=[]):
        super(SPP_NET_AC, self).__init__()
        self.gpu_ids = gpu_ids
        self.output_num = [4, 2, 1]
    ...
    def forward(self, x):
        ...
        # print(x.size())
        spp = self.spatial_pyramid_pool(x, 1, [int(x.size(2)), int(x.size(3))], self.output_num)
        spp=spp.view(self.batch_size,21)
        '''
        B,cc=spp.size()
        ...
        if cc<21:
            try:
                ...
                dd=21-cc
                tp=spp[:,cc-dd:cc]
                spp=torch.cat((spp,tp),1)
                #print(cc,spp.size())
        '''
        out_bg=self.classify(spp)
        #out_bg= F.relu(out_bg)
        out_bg=self.ac(out_bg)
        spp=spp.view(1,-1)
        # print(spp.size())
        # fc1 = self.fc1(spp)
        # fc2 = self.fc2(fc1)
